...
class PlanillaProcessor(BaseDocumentProcessor):
    ...
    def process_file(self, file_path: str) -> Optional[Dict[str, pd.DataFrame]]:
        ...
        try:
            with open(file_path, 'rb') as f:
                file_content = f.read()
            return self._process_zip_bytes(file_name, file_content)
    ...
    def process_content(self, file_name: str, file_content: bytes) -> Optional[Dict[str, pd.DataFrame]]:
        self.log_operation("Procesamiento Planilla", "Iniciado", f"Archivo (memoria): {file_name}")
        try:
            return self._process_zip_bytes(file_name, file_content)
        except Exception as e:
            self.log_operation("Procesamiento Planilla", "Error", f"Error inesperado en {file_name}: {e}", level=logging.ERROR)
    ...
    def _process_zip_bytes(self, file_name: str, file_content: bytes) -> Optional[Dict[str, pd.DataFrame]]:
