"""
FacturaProcessor - Processes SUNAT electronic invoices (UBL 2.1 XML).
...
from src.utils.cui_generator import build_cui_comprobante
...
class FacturaProcessor(BaseDocumentProcessor):
    ...
    def process_file(self, file_path: str) -> Optional[Dict[str, pd.DataFrame]]:
        ...
        try:
            ...
            return self._parse_xml(xml_content, file_name)
    ...
    def process_content(self, file_name: str, file_content: bytes) -> Optional[Dict[str, pd.DataFrame]]:
        ...
        try:
            ...
            # Case 1: ZIP file - extract XML from bytes
    ...
    # ── Core XML parsing logic (shared by both interfaces) ────────────────

    def _parse_xml(self, xml_content: str, file_name: str) -> Optional[Dict[str, pd.DataFrame]]:
        ...
        result = {
            ...
            'despatch_references': pd.DataFrame()
        }
            ...
            for tax_subtotal in tax_total.findall('.//cac:TaxSubtotal', self.NAMESPACES):
                ...
                if tax_scheme is not None:
                    ...
                    if tax_code == '1000':
                        ...
                        invoice_data['total_valor_venta'] = taxable_amount
                    elif tax_code == '2000':
                        invoice_data['total_isc'] = tax_amount
                    elif tax_code == '9999':
                        invoice_data['total_otros_tributos'] = tax_amount
                    elif tax_code == '9997':
                        invoice_data['total_exonerado'] = taxable_amount
                    elif tax_code == '9998':
                        invoice_data['total_inafecto'] = taxable_amount
                    elif tax_code == '9996':
                        invoice_data['total_gratuito'] = taxable_amount
        ...
        # Generate CUI using the centralized generator
        cui = build_cui_comprobante(
            invoice_data.get('ruc_emisor'),
            invoice_data.get('tipo_documento'),
