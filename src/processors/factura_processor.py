    ...
    def process_file(self, file_path: str) -> Optional[Dict[str, pd.DataFrame]]:
        ...
        try:
            ...
            if ruc_emisor and tipo_doc and numero_factura:
                try:
                    ruc_hex = hex(int(ruc_emisor))[2:].upper()
                    tipo_doc_fmt = f"{tipo_doc:0>2}"
                    numero_fmt = numero_factura.replace('-', '')
                ...
                self.log_operation("Procesamiento", "Advertencia", f"Faltan datos para generar CUI en archivo: {file_name}. RUC: {ruc_emisor}, TipoDoc: {tipo_doc}, Numero: {numero_factura}")
            ...
            for pt_node in payment_terms_nodes:
                ...
                pt_data['medio_pago_id'] = self.safe_find_text(pt_node, './cbc:PaymentMeansID', self.NAMESPACES)
            ...
            self.logger.debug(f"Retornando diccionario para {file_name}. Header empty: {is_header_empty}")
            return result
            
        except ET.ParseError as e_parse:
            self.log_operation("Procesamiento", "Error", f"Archivo XML mal formado: {file_name}, Error: {e_parse}")
            self.logger.debug(f"Retornando None debido a ParseError para {file_name}.")
            return None
